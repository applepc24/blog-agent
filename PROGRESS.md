...
- WordPress 마크다운 변환 개선: fenced_code + tables 확장 추가 (코드블록/표 정상 렌더링)
- 수정 기능 개선: revise 프롬프트 강화(수정 요청 우선 배치, "반드시 모두 반영" 명시) + max_tokens 4096→8192 + 수정 후 미리보기 300자 디스코드 표시

## 다음
- 글 30개까지 꾸준히 작성 (지금 22개) - 이게 최우선
...
- 조율자(coordinator) 에이전트는 규모 커지면 추가 예정ㅇ
